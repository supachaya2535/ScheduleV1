...
namespace AppointmentQueue
{
    public partial class Form1 : Form
    {
        ...
        private void dataGridView2_RowHeaderMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            scan_CoBox.SelectedIndex = Convert.ToInt32(dataGridView2.Rows[ID].Cells[9].Value.ToString()) + 1;
        ...
        private DataTable readAppointmentToday()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT ap_id,ap_piority,ap_startT,ap_duraT,ap_patient,ap_request,ap_paid," +
                "ap_patientstatus,ap_scan,ap_scannum FROM Appointments WHERE ap_startT >= @StartT AND ap_startT < @EndT ORDER BY ap_piority", cn);
        ...
        private DataTable readAppointment()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT ap_piority,ap_startT,ap_duraT,ap_patient,ap_request,ap_paid," +
                "ap_patientstatus,ap_scan,ap_scannum FROM Appointments", cn);
            SqlDataReader reader = command.ExecuteReader();
        ...
        private void addBtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string sql = "INSERT INTO Appointments (ap_piority,ap_startT,ap_duraT,ap_patient,ap_request,ap_paid,ap_patientstatus,ap_scan,ap_scannum) " +
                    "VALUES(@ap_piority,@ap_startT,@ap_duraT,@ap_patient,@ap_request,@ap_paid,@ap_patientstatus,@ap_scan,@ap_scannum)";
                SqlCommand exeSql = new SqlCommand(sql, cn);

                int scannum = 1;
                //DataTable dt = readAppointment();
                DataTable dt2 = readAppointmentToday();
                Appointment app = new Appointment(todayDatePicker.Value.Date, HNtxt.Text, reqCob.Text, paidCob.Text, birthDatePicker.Value.Date, statusCob.Text, scan_CoBox.Text, scannum,dt2);
